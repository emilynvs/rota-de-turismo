...
def dijkstra(pontos, inicio, fim):
    ...
    print(f"A distância total da rota é: {distancias[fim]} unidades.")
    return rota[::-1]
...
def verificacao(resultado):
    match resultado:
        ...
        case '6':
            return 'centro cultura de São Francisco'
        case _:
            return False       

pontos = {
    'praia de tambaú': [('jardim botânico', 6.4), ('piscinas naturais de picãozinho', 0.11), ('mercado de artesanato paraibano', 0.7), ('centro cultura de São Francisco', 7)],
    'mercado de artesanato paraibano': [('jardim botânico', 7.9), ('praia de tambaú', 0.7)],
    'piscinas naturais de picãozinho': [('praia de tambaú', 0.11), ('centro cultura de São Francisco', 7.8)],
    'jardim botânico': [('mercado de artesanato paraibano', 7.9), ('praia de tambaú', 6.4), ('parque sólon de lucena', 3.2), ('centro cultura de São Francisco', 4.9)],
    'parque sólon de lucena': [ ('jardim botânico',3.2), ('centro cultura de São Francisco', 2)],   
...
def desenhar_grafo(pontos, rota):
    G = nx.Graph()
    for no in pontos:
        G.add_node(no)
        for arestas in pontos[no]:
            G.add_edge(no,arestas[0])
    lista_de_arestas = [(rota[i], rota[i+1]) for i in range(len(rota)-1)]
    pos = nx.spring_layout(G)
    nx.draw(G, pos, with_labels=True)
    nx.draw_networkx_nodes(G, pos, nodelist= rota, node_color='lightcoral')
    nx.draw_networkx_edges(G, pos, edgelist = lista_de_arestas, edge_color = 'lightcoral', width = 5)
    plt.show()
...
\n6 - Centro Cultura de São Francisco")
